...
Нажмите на кнопку ниже, чтобы скачать автономный EXE. Python, архив и
дополнительные папки не нужны:

👉 **[СКАЧАТЬ ПОСЛЕДНЮЮ РЕЛИЗНУЮ ВЕРСИЮ (GitHub Releases)](https://github.com/ryoqe/gibvpn/releases/latest)**

Или прямая ссылка на последний файл:
- ⚡ [GibVPN_Smart_v3.exe](https://github.com/ryoqe/gibvpn/releases/latest/download/GibVPN_Smart_v3.exe)

### 📂 Инструкция по запуску:
1. Скачайте **`GibVPN_Smart_v3.exe`**.
2. Запустите файл. Для режима «Полный VPN (TUN)» подтвердите запрос администратора.
3. Одновременно держите включённым только один полный VPN-клиент.
...
- **🔄 Автообновления через GitHub**:
  - Автоматическая проверка и загрузка новых релизов с обязательной проверкой SHA-256 от GitHub.
- **🌐 Полный VPN (TUN)**:
...
- **⚙️ Актуальные сетевые ядра**:
  - Xray-core 26.7.28 и sing-box 1.13.14 встроены в автономный EXE и обновляются вместе с приложением.
...
build.bat
```
Скрипт проверит SHA-256 официального Xray и создаст один файл:
`dist\GibVPN_Smart_v3.exe`.
